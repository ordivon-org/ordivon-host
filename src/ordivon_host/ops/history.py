...
_CORE_CAS_DIGEST_KEYS = frozenset(
    {
        "planDigest",
        "effectDigest",
        "bindingDigest",
    ...
    def visit(current: JsonValue) -> None:
        if isinstance(current, dict):
            for key, item in current.items():
                if (
                    key in _CORE_CAS_DIGEST_KEYS or key.endswith("ObjectDigest")
                ) and isinstance(item, str):
                    found.append((key, item))
                if key.endswith("ObjectDigests") and isinstance(item, list):
                    for digest in item:
...
def _validate_core_semantic_links(
    storage: HostStorage,
    data: JsonValue,
    event_id: str,
