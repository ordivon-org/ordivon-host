...
def build_mcp_server(settings: HostMcpSettings) -> MCPServer:
    ...
    @server.tool(
        ...
        description=(
            ...
            "Event payload data and never invokes Runtime, Harness, or a Provider. Its checkpoint "
            "fields are bounded previews, not a semantic write base: before replacing synthesized "
            "WorkingCheckpoint fields, use task.resume at the exact current revision to consume the "
            "complete checkpoint. This read validates Journal invariants plus the CAS objects it "
            "consumes; unrelated CAS health remains a global status/Doctor concern."
        ),
        annotations=ToolAnnotations(
            readOnlyHint=True,
            destructiveHint=False,
    ...
    @server.tool(
        ...
        description=(
            ...
            "automatic current work admission or priority. This is the full semantic-base read to "
            "use before synthesizing or replacing WorkingCheckpoint fields; task.observe/task.list "
            "return only bounded previews. expectedRevision is an optional stale-read fence. Any "
            "retained Runtime hint is navigation only and does not authorize Workspace "
    ...
    @server.tool(
        ...
        description=(
            ...
            "fields only for continuityDisposition=continue; present list-valued fields are whole-list "
            "replacements, not append/merge operations. expectedRevision prevents stale admission but "
            "does not prove semantic-base consumption: before replacing synthesized shared standing, "
            "task.resume the exact current full checkpoint and merge deliberately. A new "
            "complete/abandon transition requires a full WorkingCheckpoint so no inherited field is "
            "frozen accidentally. "
