...
```text
host.sqlite3   schema v8 event journal, Task projection, namespaced extension-state pointers, leases, event-object references, durable board/news publication pointers, and CAS validation cache
...
- v4 → v5 adds `task_extension_state`, a schema-blind per-Task/per-Event-namespace pointer to opaque extension state. New/current extension writes retain one `host-extension-state` CAS object atomically with the Event and current Task revision; ordinary Host core Events do not overwrite that namespace state. `HostExtensionPort.load_namespace()` therefore recovers native v5 owner state across later checkpoints or other extension namespaces while preserving the current `TaskProjection`. Starting with Host 0.5, a schema-v4 authority containing any extension Event is **not** auto-interpreted or backfilled: opening it fails closed and requires a pre-0.5 Host client to recover/export that owner's historical state before upgrade. A v4 authority with no extension Events still migrates to the same v5 schema and an empty namespace table. The `legacy` storage/snapshot field remains for wire/schema compatibility but supported 0.5 rows must be `false`/`0`; a nonzero marker fails closed. The old `recover_legacy_namespace()` executable compatibility path is retired. The source database backup remains `host.sqlite3.pre-schema-v5.sqlite3`.
- v5 → v6 adds `board_messages`, an append-ordered Host-global collaboration index whose message bodies remain immutable `host-board-message` CAS objects. `client_message_id` is exact replay identity, author labels are explicitly self-asserted rather than authenticated identity, and reply references are admitted only to existing board messages. Board persistence/order does not grant Task priority, execution authority, owner standing, or domain truth. The source database is retained as `host.sqlite3.pre-schema-v6.sqlite3`.
- v6 → v7 adds `object_refs.validation_timing`. Existing v6 references begin as `startup`; Board-message objects are reclassified to `on_access` only when Board is their sole durable use, while any Event/legacy/extension use keeps the stronger startup classification. New Board messages are admitted as `on_access`, and any later startup-critical use promotes the retained object to `startup` without downgrade. This preserves already-materialized v6 authorities while separating the complete retention/backup/GC inventory from ordinary startup validation. The source database is retained as `host.sqlite3.pre-schema-v7.sqlite3`.
- v7 → v8 adds `news_editions` and `news_publications`, a revision-fenced daily external-news publication projection. One complete structured `NewsEdition` is retained as one immutable `host-news-edition` CAS object per revision and admitted as `on_access`; ordinary Host startup validates cheap Journal edition/revision/head relations but does not enumerate historical news payload bytes. `client_publish_id` is exact replay identity and `expected_revision` fences corrections. News persistence proves only that a producer published exact bytes and source pointers; it does not make external claims World/owner/domain truth and does not create Tasks or Board messages. The source database is retained as `host.sqlite3.pre-schema-v8.sqlite3`.

A v1 database advances through all migrations in order and records every transition in `schema_migrations`.

Schema realization belongs to bootstrap and genuine forward migration only. An already-current Journal is validation-only: its metadata, complete owner table/index/trigger/view set, and normalized schema definitions must match the canonical current shape, except that a validated contiguous v1/v2 compatibility lineage may match one complete exact legacy core-table class when it actually differs from canonical; canonical current shape remains valid for every lineage, and partial legacy/canonical mixtures are rejected. Missing or changed current schema objects fail closed and are never recreated by ordinary reopen. This keeps observation off SQLite writer locks without hiding authority evidence loss behind self-heal.
...
`object_refs` is the complete retention inventory used by backup/restore/GC. Schema v7+ separately records when a retained object must be validated:

- `startup`: Task/Event/extension and other Host-core references. Ordinary Host open enumerates this subset; missing objects fail immediately, unchanged verified file identity may reuse the validation cache, and any file-identity change/cache miss forces full decode + SHA-256 verification. Every materialized Task head is still read and rebuilt against its exact Event payload.
- `on_access`: currently `host-board-message` and `host-news-edition` history. Ordinary Host open does **not** enumerate or open these historical payload files. A `(validation_timing, digest)` index keeps this selection proportional to startup-critical references rather than total retained history, and the CAS store constructor does not perform a whole-directory permission sweep. File mode is hardened when a retained object is actually validated/accessed; explicit Doctor scans and hardens the complete object directory, including orphans. `board.list` and `news.read` validate the exact reference classification and immutable CAS they return, so corruption fails closed at the owning access boundary without making unrelated Host operations unavailable.

This is a validation-timing distinction, not a retention or integrity distinction. `ordivon-host doctor` uses full validation over **every** retained `object_ref` regardless of timing, then performs Board structural/semantic checks including append-only sequence continuity, reply integrity, reference classification, and row ↔ immutable-message identity. Doctor, `inspect`, standalone `gc plan`, and CLI `task list|show|handoff|resume` deliberately do not refresh `object_validation`: these observation/planning paths remain read-only with respect to Journal validation-cache state, so a concurrent WAL writer is not blocked merely to warm a performance cache. Cache-enabled full validation first exhausts its reference-validation read snapshot and only then writes verified file identities back in bounded transactions; it never upgrades a still-active reference cursor into a writer, avoiding `SQLITE_BUSY_SNAPSHOT` between overlapping validators. Backup uses a stronger snapshot-relative boundary: it first freezes one standalone SQLite backup, enumerates the exact `object_refs` from that frozen Journal, copies only those CAS objects, and full-validates the assembled temporary authority before publication. Facts committed after the SQLite snapshot belong to the next backup; a missing or corrupt CAS referenced by the frozen snapshot blocks creation rather than allowing a backup that later fails verification. Backup verification independently re-derives Journal schema, migration history, and exact object-reference inventory from the frozen database, requires the root and object-store filesystem topology to contain only the corresponding regular files with no symlink, nested, or orphan entries, and compares the declared file manifest with those exact bytes; restore copies only that verified reference set. Manifest projection or filesystem-topology drift therefore fails closed instead of becoming trusted metadata. The validation cache remains scoped to the trusted-local filesystem threat model.
...
For reproducible release construction, `prepare` derives `SOURCE_DATE_EPOCH` from the exact source Commit timestamp before building the wheel. After project installation it removes installer-local `direct_url.json` and `uv_cache.json` metadata and removes their rows from the installed `RECORD`; these values describe temporary build paths/times rather than executable state. Repeating `prepare` for the same source Commit, lockfile, toolchain, and Python runtime must therefore converge to the same effective release bytes and `releaseId`, while `builtAtMs` remains candidate-receipt metadata outside the release-tree identity.
...
Deployment receipts under `/var/lib/ordivon/host/deployments/` are evidence-retained and are never automatically removed by Host lifecycle GC. Their exact directory trees participate in the lifecycle plan digest, so receipt drift between planning and application invalidates the plan. Historical deployment-incident investigations explicitly named `incident-*` may coexist in that evidence root; they are retained and tree-bound as `incident_evidence_retained`, but they are not deployment receipts and therefore do not need a receipt `manifest.json`. This exception is name-scoped to the incident evidence namespace: any ordinary deployment-receipt directory with a missing or unreadable manifest still blocks lifecycle planning.
...
H-C2 projects Host authority over a small Agent-native MCP endpoint with eleven Tools. Seven are read-only: `host.status`, `board.list`, `news.list`, `news.read`, `task.observe`, `task.list`, and `task.resume`. Four are writes with narrow responsibility: `board.post` persists collaboration messages only, `news.publish` persists one revision-fenced external-news edition only, while `task.adopt` and `task.checkpoint` mutate only external-continuity state. Each request reopens Host authority rather than persisting transport session state; the MCP server does not own a second Task store, Runtime connection, Harness Run, scheduler, or Provider session.

Validation timing and operation scope are separate dimensions. Schema v7 `startup|on_access` determines when a retained object participates in a **global** authority-health pass; it does not imply that every local read must run that global pass first. `board.list`, `news.list`, `news.read`, `task.observe`, `task.list`, and `task.resume` therefore use operation-local validation: they still open the Host Journal, validate schema/migration compatibility and global relational Journal invariants, and fully decode/hash/kind-check every CAS object actually consumed by that operation, but they do not stat/hash every unrelated `startup` reference or rebuild every unrelated Task head before returning. Their MCP CallToolResult metadata carries `_meta["ordivon/hostIntegrityScope"].scope=operation-local` and `globalCasHealthClaimed=false`; the durable/semantic `structuredContent` remains unchanged. A missing or corrupt object required by the requested Board/News/Task view still fails closed. `host.status`, ordinary non-targeted `HostStorage`, write admission, deployment probes, backup/restore and Doctor retain global validation semantics. Thus an unrelated corrupt retained object may be deferred by a valid local read, but it is not reclassified as healthy or silently removed from global integrity obligations.

`host.status` is the compact top-level observation entry. `detail=summary` returns Journal/schema/task counts, continuity counts, bounded recent Task activity, and deployment identity only when the running MCP module is actually loaded from the installed current release. Summary is deliberately **startup-global**, not a claim that every retained CAS object is healthy: its MCP metadata reports `globalCasHealthClaimed=false` because `on_access` Board/News history is deferred to its owning access boundary. `detail=integrity` adds the existing full local Host Doctor and may claim global retained-CAS health only after that Doctor succeeds; `detail=history` additionally validates retained Event history. If either global validation call fails, the returned error metadata explicitly cannot retain `globalCasHealthClaimed=true`. It never proxies Runtime. Every Host Tool result, including structured errors, also carries `serverInterface`: the surface version, exact Tool names, and a canonical `schemaDigest` / `schemaRevision` over Tool name + input schema + the output schema actually published by MCP discovery. Titles and descriptions are deliberately excluded so presentation edits do not masquerade as interface changes. An undeclared Tool output contributes `null`; therefore `schemaDigest` must not be interpreted as binding structured output that the Tool does not publish. `board.list` is the first Host Tool to publish and runtime-validate its structured success/error output through MCP 2.0 `outputSchema`; the remaining Tools retain input-only schema identity until their result contracts are independently hardened. A client can compare the identity attached to an ordinary Tool result with a freshly observed wire `tools/list`; a mismatch proves the declared interface differs without asking Host to guess client state.

`board.post` appends one bounded Host-global collaboration message. `clientMessageId` is caller-chosen exact replay identity: retrying identical content converges, while reusing that identity for different content fails closed. `authorLabel` is deliberately self-asserted and must not be treated as authenticated dialogue/Agent identity. Optional `replyToClientMessageId` is a durable relation to an already admitted message, not an endorsement or authority edge. Board CAS is retained in the common Host object inventory with `validation_timing=on_access`; `board.list` validates the exact row/reference/CAS state of the bounded messages it returns, while ordinary Host open does not scan Board history. Omitting `afterSequence` returns the newest bounded window in chronological order; passing an observed sequence performs incremental polling for later messages. Optional `topic` and `replyToClientMessageId` are exact retrieval filters over already-durable Board relations only. The reply filter returns direct children of one exact admitted message; it does not infer recursive thread ancestry, endorsement, authority, voting identity, or presence. These filters add no ranking, semantic inference, supersession, or authority. A filtered read captures the global append-only sequence high-water before selecting rows and restricts selection to that prefix. When a filtered incremental page exhausts matching rows, `nextAfterSequence` advances to the captured high-water so unrelated traffic is not rescanned; when matching rows are actually truncated by `limit`, `hasMore=true` and the cursor remains at the last returned match until that captured prefix is drained. `messageCount` and `lastSequence` on a filtered response describe the same captured prefix, so a concurrent append belongs to the next poll rather than creating internally mixed snapshot metadata. Board admission is append-only, so the sequence high-water mark is the cheap current message count; Doctor explicitly proves sequence continuity before that invariant is trusted as historical integrity. Board messages are not Tasks, current-work priority, execution permission, owner standing, or domain truth. Consumers must revalidate any owner/world/source claim at its actual authority before acting.

`news.publish` persists one complete structured daily external-news edition under caller-chosen `clientPublishId` exact replay identity and an `expectedRevision` fence. A correction creates a new immutable edition revision; prior revisions remain readable. News CAS is retained with `validation_timing=on_access`; ordinary Host open validates only cheap Journal publication/revision/head relations, while `news.read`, Doctor and backup validate payload bytes at their owning boundary. `news.read` returns the latest or one exact revision and can filter that bounded edition by section, category, or producer-supplied thread key; the long human-readable brief is omitted unless explicitly requested. `news.list` discovers edition headers by date with a query-bound cursor. Durable evidence uses source URLs, DOI/arXiv/regulatory/official identifiers or equivalent source identities; ephemeral conversation citation IDs are not publication provenance. `news.*` does not research the world, semantically canonicalize stories, grant source reliability, create Tasks, or promote a retained claim into owner/World truth. See [`DAILY_NEWS.md`](DAILY_NEWS.md).

`task.observe` is the compact per-Task observation entry for any Host Task. It returns the exact projection, workload identity, current head metadata, bounded recent Event timeline, handoff capsule, and either continuity summary or Host recovery assessment. For external-continuity checkpoint revisions it also projects the optional durable `writerLabel` plus `writerIdentityRole=self-asserted-label`; older unlabeled revisions are reported as `unrecorded`. It omits raw Event payload data and never invokes Runtime/Harness/Provider. Its checkpoint content is deliberately a bounded preview, not a semantic write base. Before replacing synthesized WorkingCheckpoint fields, use `task.resume` at the exact current revision and consume the complete checkpoint; knowing only the revision or preview/digest is insufficient to preserve peer semantics.

`task.list` defaults to non-terminal `ordivon.host.external-continuity.v1` Tasks only; unrelated Host workloads cannot consume the external Agent's discovery window. It is a continuity inventory, not a cross-owner current-work or priority surface. For this workload, `projection.state=ready` means only that Host continuity remains open at its synthetic continue frontier; owner-relative dispositions such as resume-now, wait, hold, superseded, current priority, standing, and domain currentness are not Host lifecycle facts. Results are ordered by immutable Task creation identity and paginated with an opaque cursor bound to the exact `goalId`, optional exact `runtimeWorkspaceId`, and `includeTerminal` query scope. `runtimeWorkspaceId` performs structural reverse-reference discovery over the current Host-retained Runtime navigation hint only: it never queries Runtime and does not establish carrier currentness, ownership, retention/closure authority, priority, or semantic claimant truth. Reusing a cursor under another scope fails closed; legacy cursors remain valid only for queries without the newer workspace filter. Each row includes the exact current Task projection and a bounded semantic checkpoint preview: objective/frontier previews are independently limited to 512 UTF-8 bytes with explicit truncation flags, while checkpoint revision/digest allow the Agent to open the full revision-coherent checkpoint with `task.resume`. Terminal continuity is hidden by default and can be inspected with `includeTerminal=true`.

The `task.adopt.initialCheckpoint` and `task.checkpoint.checkpoint` Tool inputs publish the complete WorkingCheckpoint JSON shape in MCP discovery, including the distinction between semantic working claim and optional physical navigation hint. Both write Tools also accept optional top-level `writerLabel`: a bounded self-asserted provenance claim stored on the admitted checkpoint Event, deliberately outside WorkingCheckpoint semantic content. It is not inherited by checkpoint patches and is not authenticated identity, Task ownership, current priority, or execution authority. Exact replay returns the writer label already persisted by the committed revision, so a later retry cannot rewrite historical provenance. Older checkpoint Events without this field remain valid and project `writerIdentityRole=unrecorded`. The schemas guide Agent generation only: handlers still apply Host-native admission validation. Request errors that Host can attribute to one Tool field report that field together with `INVALID_ARGUMENT`, `retryClass=fix_request`, and `commitState=not_committed`.

`task.checkpoint` accepts either a complete `ordivon.host-working-checkpoint` or a revision-bound patch in the same `checkpoint` argument. Patch keys are limited to `objective`, `frontier`, `established`, `unresolved`, `rejected`, `constraints`, `nextActions`, and `runtime`; omitted fields are inherited from the exact `expectedRevision`, while present fields replace that complete field value. In particular, list-valued fields are whole-list replacements, not append/merge operations. `expectedRevision` prevents a stale concurrent transition but does not prove that the writer consumed or semantically merged the current WorkingCheckpoint. For synthesized shared standing, call `task.resume` at the exact current revision, deliberately merge the complete semantic base, then checkpoint the resulting replacement. Additive multi-dialogue findings normally belong on Board rather than in concurrent list patches. Host reconstructs and validates a complete canonical WorkingCheckpoint before Journal admission, so patching changes transport ergonomics rather than durable semantics. The same call accepts `continuityDisposition=continue|complete|abandon`, defaulting to `continue`. A patch may advance only open continuity (`continue`). A new `complete` or `abandon` transition must provide the full checkpoint so every field is explicitly adjudicated before the immutable terminal state is committed. An exact replay of an already committed terminal transition still converges, including a legacy patched request. `complete` and `abandon` terminate only Host continuity tracking; they do not assert external domain success or failure.
...
Backup verification is full and read-only with respect to the evidence directory. Exact manifest-covered bytes are checked in place, but semantic validation runs only against a disposable state copy so opening an older Journal may exercise current schema migrations, permission hardening, and validation-cache logic without modifying the backup being verified. Repeated verification must therefore preserve the original database digest and file set even across schema upgrades. Backup creation publishes with Linux `renameat2(RENAME_NOREPLACE)`, so a destination that appears at any point before commit is preserved rather than silently replaced. Restore first copies the caller backup exactly once into a private staged evidence image using no-follow regular-file descriptors and the staged Journal's exact object-reference inventory, fsyncs and verifies that staged image, then never reads the caller path again. It realizes a temporary target state only from the verified staged evidence, opens that realization under current full Host invariants (including any admitted forward migration), fsyncs it, and only then atomically renames the realized state into place; a caller path that changes from valid backup A to valid backup B after verification therefore cannot produce receipt A with realized state B. `--replace` preserves the old state root under a timestamped `.previous-*` path. Without `--replace`, final publication uses the same Linux `renameat2(RENAME_NOREPLACE)` boundary: target absence is therefore enforced atomically at commit rather than by a check-then-rename window. If another actor materializes the target at any point before that commit, restore fails closed and the late target is not moved aside as an implicit replacement. If this primitive is unavailable, non-replacing publication fails closed rather than falling back to a racy check-and-replace sequence.
