...
An already-current Journal whose required current tables, named indexes, and metadata keys are present opens without replaying schema DDL. Fresh initialization, forward migration, and repair of a missing current schema object still execute the canonical schema realization path. This separates current-schema validation from schema construction so an ordinary read can coexist with an independent WAL writer without weakening migration convergence or current-schema repair.
...
This is a validation-timing distinction, not a retention or integrity distinction. `ordivon-host doctor` uses full validation over **every** retained `object_ref` regardless of timing, then performs Board structural/semantic checks including append-only sequence continuity, reply integrity, reference classification, and row ↔ immutable-message identity. Doctor deliberately does not refresh `object_validation`: it re-hashes the complete retained set for evidence but remains read-only with respect to Journal validation-cache state, so a concurrent WAL writer is not blocked merely to warm a performance cache. Cache-enabled full validation (including backup admission) first exhausts its reference-validation read snapshot and only then writes verified file identities back in bounded transactions; it never upgrades a still-active reference cursor into a writer, avoiding `SQLITE_BUSY_SNAPSHOT` between overlapping full validators. Backup still full-validates the source authority before copying any CAS object; a corrupt/missing on-access Board payload therefore blocks backup rather than being silently preserved. The validation cache remains scoped to the trusted-local filesystem threat model.
