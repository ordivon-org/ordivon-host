...
```text
host.sqlite3   schema v7 event journal, Task projection, namespaced extension-state pointers, leases, event-object references, durable board-message pointers, and CAS validation cache
...
- `on_access`: currently `host-board-message` history. Ordinary Host open does **not** enumerate or open these historical payload files. A `(validation_timing, digest)` index keeps this selection proportional to startup-critical references rather than total retained history, and the CAS store constructor does not perform a whole-directory permission sweep. File mode is hardened when a retained object is actually validated/accessed; explicit Doctor scans and hardens the complete object directory, including orphans. `board.list` validates the exact reference classification and immutable CAS of each message it returns, so corruption fails closed at Board access without making unrelated Host Task operations unavailable.
...
H-C2 projects Host authority over a small Agent-native MCP endpoint with eight Tools. Five are read-only: `host.status`, `board.list`, `task.observe`, `task.list`, and `task.resume`. Three are writes with narrow responsibility: `board.post` persists collaboration messages only, while `task.adopt` and `task.checkpoint` mutate only external-continuity state. Each request reopens Host authority rather than persisting transport session state; the MCP server does not own a second Task store, Runtime connection, Harness Run, scheduler, or Provider session.
