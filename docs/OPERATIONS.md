...
- `on_access`: currently `host-board-message` history. Ordinary Host open does **not** enumerate or open these historical payload files. `board.list` validates the exact reference classification and immutable CAS of each message it returns, so corruption fails closed at Board access without making unrelated Host Task operations unavailable.
