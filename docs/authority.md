...
Source code, schema migrations, deterministic tests, exact dependency pins, live service inspection, and immutable receipts remain stronger owners for fields, transitions, compatibility, performance measurements, and deployed state. `CLOSURE.md`, extraction records, P/H/Round reports, remediation notes, and evidence files explain how the current boundary was reached; they do not silently redefine it.

## Consequences

Only the repository entry, architecture, operations contract, and this decision enter strict content management in this adoption step. Historical and phase-oriented texts remain available without bulk metadata conversion. A later human-centered rewrite may reorganize and simplify them, but it must preserve evidence references and use explicit supersession rather than creating another current Host truth.
